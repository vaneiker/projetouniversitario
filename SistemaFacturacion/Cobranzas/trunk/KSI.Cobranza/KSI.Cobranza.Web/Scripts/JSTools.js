...
//Modal de BootStap   
function ShowMessageBS(message, title, OkCallBack, IsCentered) {
...
//Popup
function ShowPopupBS(content, title, CallBackCloseButton, IsCentered) {
    IsCentered = (IsCentered === undefined) ? false : IsCentered;
    var $myModal = $("#myModal");
...
//Popup Confirm  
function ModalConfirm(message, YesCallBack, NoCallBack, IsCentered) {
    IsCentered = (IsCentered === undefined) ? false : IsCentered;
    ...
    $body.html(message == undefined ? "¿Esta seguro de realizar esta operación?" : message);
    $(".btn-primary").off('click');
    $(".btn-primary").click(function () {
        $myModal.modal('hide');
...
function CallAjax(vUrl, vParameter, vDataType, vSucess, RequestType, isAsync) {
    $.ajax({
        ...
        error: function (data) {
            if (data.status == 0) {
                ShowMessageBS("Se perdido la conexión con el servidor remoto", "Advertencia");
                return false;
            }
            if (data.status != 200) {
