...
namespace WSVirtualOffice.Models.blinsurance
{
    public class IMainfuneral
    {
        ...
        private char repatriacion;
        private char SepulturaLote;
        private char familiar;
        ...
        public double regularinsurancecostbasic;
        ...
        public IMainfuneral(long customerplanno)
        {
            ...
            try
            {
                ...
                repatriacion = custplan.Repatriacion;
                SepulturaLote = custplan.SepulturaLote;
                familiar = custplan.Familiar;
                ...
                if (custplan.riderterm != null)
                {
                    ...
                    this.setRiderfuneraldata(Booleandata.getBooleanstring(custplan.riderterm), custplan.ridertermamount.ToString(), temptermnumyears.ToString());
        ...
        public IMainfuneral(WSCustomer cust, WSCustomerPlanFuneral custplan)
        {
            DataVOUniversalDataContext newdb = Program.getDbConnection();
            try
            {
                custplan.insuredamount = Rules.getRulevaluedouble(Rules.FUNERAL_INSURED_AMOUNT, custplan.productcode, custplan.classcode.ToCharArray()[0]);
                custplan.calculatetypecode = "P";
                ...
                isrefund = Productdata.isRefund(this.productcode);
                repatriacion = (custplan.Repatriacion == null ? 'N' : custplan.Repatriacion.ToCharArray()[0]);
                SepulturaLote = (custplan.SepulturaLote == null ? 'N' : custplan.SepulturaLote.ToCharArray()[0]);
                familiar = (custplan.Familiar == null ? 'N' : custplan.Familiar.ToCharArray()[0]);
                ...
                this.retirementnoofyears = custplan.retirementperiod;
                this.defermentnoofyears = custplan.defermentperiod;
        ...
        public void setRiderfuneraldata(String riderterm, String ridertermamountstr, String ridertermuntilagestr)
        {
        ...
        public double goalseekfunerary(double insuredamount)
        {
            return getAnnualpremiumregular(insuredamount, true);
        }
        ...
        */
        ...
        public double getVariableisuranceamount(int sno)
        {
            ...
            if (repatriacion == 'Y')
                tempyearlyinsuredamount = tempyearlyinsuredamount + Rules.getRulevaluedouble(Rules.REPATRIATION, this.productcode, this.clascode);
            /*if (familiar == 'Y')
                tempyearlyinsuredamount = tempyearlyinsuredamount * Rules.getRulevaluedouble(Rules.FAMILIARYES, this.productcode, this.clascode);
            else*/
            if (familiar == 'N')
                tempyearlyinsuredamount = tempyearlyinsuredamount * Rules.getRulevaluedouble(Rules.FAMILIARNO, this.productcode, this.clascode);

            if (SepulturaLote == 'P')
                tempyearlyinsuredamount = tempyearlyinsuredamount + Rules.getRulevaluedouble(Rules.PUBLICO, this.productcode, this.clascode);
            else if (SepulturaLote == 'R')
                tempyearlyinsuredamount = tempyearlyinsuredamount + Rules.getRulevaluedouble(Rules.PRIVADO, this.productcode, this.clascode);
        ...
        public Termillusdata[] getIllustration()
        {
            ...
            for (int i = 0; i < retirementnoofyears; i++)
            {
                ...
                //Comentado por Ordenes de German, el rider no debe ser calculado.
                //tempillusdata[i].Totalpremium = tempillusdata[i].Regularpremium + tempriderpremiumamount;
                tempillusdata[i].Totalpremium = tempillusdata[i].Regularpremium;
            }
        ...
        public double getTargetamount()
        {
            ...
            //if (productcode == "ORN" || productcode == "LUM" || productcode == "LUV")
            //    addval = (addval > 350 ? addval : 350);
            //else if (productcode == "GRD")
            //    addval = ((addval) > 350 ? addval : 350);
            //else if (productcode == "ORP")
            //    addval = (addval > (this.clascode == 'R' ? 2250 : 50) ? addval : (this.clascode == 'R' ? 2250 : 50));
            //else if (productcode == "GRP")
            //    addval = Math.Round((addval) > 2250 ? (addval) : 2250);

            //double addval1 = getTotalannualriderpremium(this.insuredamount) + 10 * numberofyears;
            //if (productcode == "ORN")
            //    addval1 = (addval1 > 300 ? addval1 : 300);
            //else if (productcode == "GRD")
            //    addval1 = (((addval1) > 300 ? addval1 : 300) + (482.403360243675 / (1 - this.Savingsmargin)));
            //else if (productcode == "ORP")
            //    addval1 = (addval1 > 2250 ? addval1 : 2250);
            //else if (productcode == "GRP")
            //    addval1 = (((addval1) > 2250 ? (addval1) : 2250) + (482.403360243675 / (1 - this.Savingsmargin)));
            // if(isrefund==false)
            // calctemptarget = Math.Min(Financial.Pmt(this.Targetrate, this.Targetcontributionperiod, Financial.PV(this.Targetrate, this.numberofyears, (this.isrefund == false) ? addval - getTotalannualriderpremiumAnnual(this.insuredamount) : addval - getTotalannualriderpremiumAnnual(this.insuredamount) + this.repaycost, 0, DueDate.BegOfPeriod) + (0.5 * Financial.PV(this.Targetrate, this.numberofyears, (getTotalannualriderpremiumAnnual(this.insuredamount)), 0, DueDate.BegOfPeriod)) - this.initialcontributionamount, 0, DueDate.BegOfPeriod), 0.95 * (this.periodicpremiumamount * this.frequencytypevalue + this.getCommissionableinicial()));
            // calctemptarget = Math.Min(Financial.Pmt(this.Targetrate, 15, Financial.PV(this.Targetrate, this.numberofyears, addval - getTotalannualriderpremium(this.insuredamount) + this.repaycost, 0, DueDate.BegOfPeriod) + ((this.isrefund == false) ? 0.5 * Financial.PV(this.Targetrate, this.numberofyears, (getTotalannualriderpremium(this.insuredamount)), 0, DueDate.BegOfPeriod) : 0) - this.initialcontributionamount, 0, DueDate.BegOfPeriod), 0.95 * (this.periodicpremiumamount * this.frequencytypevalue) + this.getCommissionableinicial());
            //  else
        ...
        public double getAnnualpremiumregular(double tempinsuranceamount, bool basic = false)
        {
            return getPeriodicpremiumregular(tempinsuranceamount, basic) * this.frequencytypevalue;
        }
        ...
        public double getPeriodicpremiumregular(double tempinsuranceamount, bool basic = false)
        {
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            if (productcode == "GRP" || productcode == "ORP")
                return Math.Ceiling(calculatePMT(Netpremiumaftermargin(tempinsuranceamount, basic), frequencytypevalue, netperiodicrate) * (1.0 + frequencytypepenalty));
            else
            {
                return Math.Ceiling(calculatePMT(Netpremiumaftermargin(tempinsuranceamount, basic), frequencytypevalue, netperiodicrate) * (1.0 + frequencytypepenalty));
        ...
        public double Netpremiumaftermargin(double tempinsuranceamount, bool basic = false)
        {
            if (getMinimumInsurancecost(tempinsuranceamount) > (getNetinsurancecost(tempinsuranceamount, basic) / (1 - this.Regularmargin) + (numberofyears * 10)))
            {
                return this.getMinimumInsurancecost(tempinsuranceamount);
            }
            else
            {
                return getNetinsurancecost(tempinsuranceamount, basic) / (1 - this.Regularmargin) + (numberofyears * 10);
            }
        }

        public double getNetinsurancecost(double tempinsuranceamount, bool basic = false)
        {
            return getTotalInsurancecost(tempinsuranceamount, basic) * (1 / ((primarymortdatatable[this.age].nx - primarymortdatatable[this.age + this.numberofyears].nx) / primarymortdatatable[this.age].ddx));
        }

        public double getTotalInsurancecost(double tempinsuranceamount, bool basicamount = false)
        {
            double totinsuredcost = 0.0;

            if (repatriacion == 'Y')
                tempinsuranceamount = tempinsuranceamount + ((basicamount) ? 0 : Rules.getRulevaluedouble(Rules.REPATRIATION, this.productcode, this.clascode));
            if (familiar == 'Y')
                tempinsuranceamount = tempinsuranceamount * ((basicamount) ? 1 : Rules.getRulevaluedouble(Rules.FAMILIARYES, this.productcode, this.clascode));
            else if (familiar == 'N')
                tempinsuranceamount = tempinsuranceamount * ((basicamount) ? 1 : Rules.getRulevaluedouble(Rules.FAMILIARNO, this.productcode, this.clascode));

            if (SepulturaLote == 'P')
                tempinsuranceamount = tempinsuranceamount + ((basicamount) ? 0 : Rules.getRulevaluedouble(Rules.PUBLICO, this.productcode, this.clascode));
            else if (SepulturaLote == 'R')
                tempinsuranceamount = tempinsuranceamount + ((basicamount) ? 0 : Rules.getRulevaluedouble(Rules.PRIVADO, this.productcode, this.clascode));
