        ...
        public IMaintermfixed(WSCustomer cust,WSCustomerPlan custplan,List<WSRider> riderslist,WSCustomerPlanPartner partnerins)
        {
            ...
            try
            {
                ...
                else if (this.calculatetypecode == Calculatetypes.PREMIUMAMOUNT)
                {
                    ...
                    asdata.insuranceamount = this.insuredamount;
                    double assumedpremiumamount = goalseek(asdata, this.GSMinimumpremiumamount, this.GSMaximumpremiumamount);
                    this.basicannualpremium = Math.Round(Netpremiumaftermargin(this.insuredamount),2);
                    //double assumedpremiumamount = goalseek(asdata, 1737.01, 1737.01);
                    this.calcyearlypremium = assumedpremiumamount;
                    this.periodicpremiumamount = this.calculatedPeriodicPremiumAmount();
                    calculatedtargetamount = getTargetamount();
                    ValuefractionSurcharge = this.fractionSurcharge = Math.Round((this.periodicpremiumamount * this.frequencytypevalue) - this.basicannualpremium, 2);
        ...
        */
        ...
        {   
            ...
            else
            {
                ...
                return Math.Round(-Financial.Pmt(netperiodicrate, frequencytypevalue, tempyearlypremiumamount, 0, DueDate.BegOfPeriod) * (1.0 + frequencytypepenalty),2);
        ...
        public double getVariableRiderperiodicPremiumamount(int sno)
        {
            ...
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            return Math.Round(-Financial.Pmt(netperiodicrate, frequencytypevalue, tempyearlyoirpremiumamount, 0, DueDate.BegOfPeriod) * (1.0 + frequencytypepenalty), 2);
        ...
        {           
            ...
            {
                temppremiumamount = Convert.ToDouble(Math.Round(periodicpremiumamount, 2));
        ...
        public double getPeriodicoirpremiumregularAnnual()
        {
            ...
            return Math.Round(calculatePMT(getNetoirpremiumaftermargin(), 1, netperiodicrate) * (1.0), 2);
        }

        public double getPeriodicaddlpremiumregularAnnual()
        {
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / 1) - 1;
            return Math.Round(calculatePMT(getNetaddlpremiumaftermargin(), 1, netperiodicrate) * (1.0), 2);
        }

        public double getPeriodicadbpremiumregularAnnual()
        {
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / 1) - 1;
            return Math.Round(calculatePMT(getNetadbpremiumaftermargin(), 1, netperiodicrate) * (1.0 ), 2);
        }

        public double getPeriodicoirpremiumregular()
        {            
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            return Math.Round(calculatePMT(getNetoirpremiumaftermargin(), frequencytypevalue, netperiodicrate), 2); // * (1.0 + frequencytypepenalty));
        }

        public double getPeriodicaddlpremiumregular()
        {            
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            return Math.Round(calculatePMT(getNetaddlpremiumaftermargin(), frequencytypevalue, netperiodicrate), 2); // * (1.0 + frequencytypepenalty));
        }

        public double getPeriodicadbpremiumregular()
        {            
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            return Math.Round(calculatePMT(getNetadbpremiumaftermargin(), frequencytypevalue, netperiodicrate),2); // * (1.0 + frequencytypepenalty));
        }
        ...
        public double getPeriodicriderpremiumaftermargin(double tempinsuranceamount)
        {
            ...
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            return Math.Round(calculatePMT(getNetriderpremiumaftermargin(tempinsuranceamount), frequencytypevalue, netperiodicrate),2);// * (1.0 + frequencytypepenalty));
        }
        public double getPeriodicriderpremiumaftermarginAnnual(double tempinsuranceamount)
        {
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / 1) - 1;
            return Math.Round(calculatePMT(getNetriderpremiumaftermargin(tempinsuranceamount), 1, netperiodicrate) * (1.0),2);
        }
        ...
        {            
           ...
           // double addval = getTotalannualpremium(this.insuredamount)+10*numberofyears;
            ...
            calctemptarget = Math.Min(
                
                Financial.Pmt(this.Targetrate, this.Targetcontributionperiod, 
                              Financial.PV(this.Targetrate, this.numberofyears, 
                                           (this.isrefund == false) ? addval - getTotalannualriderpremiumAnnual(this.insuredamount) 
                                                                    : addval - getTotalannualriderpremiumAnnual(this.insuredamount) + this.repaycost
                                          , 0, DueDate.BegOfPeriod) 
                                         + (0.5 * Financial.PV(this.Targetrate, this.numberofyears, (this.isrefund == false) ? (getTotalannualriderpremium(this.insuredamount)) : 0, 0, DueDate.BegOfPeriod)) - this.initialcontributionamount, 0, DueDate.BegOfPeriod)
           ...
           */
            return Math.Round(calctemptarget * this.Targetfactor, 2);
        }
        ...
        public double getPeriodicpremiumregularAnnual(double tempinsuranceamount)
        {
            ...
            if (productcode == "GRP" || productcode == "ORP")
                return Math.Round(calculatePMT(Netpremiumaftermargin(tempinsuranceamount) , 1, netperiodicrate) * (1.0 + 0),2);
            else
            {
                return Math.Round(calculatePMT(Netpremiumaftermargin(tempinsuranceamount) , 1, netperiodicrate) * (1.0 + 0),2);
        ...
        public double getPeriodicpremiumregular(double tempinsuranceamount)
        {
            double netperiodicrate = Math.Pow((1 + growthrate), 1.0 / frequencytypevalue) - 1;
            if (productcode == "GRP" || productcode == "ORP")
                return Math.Round(calculatePMT(Netpremiumaftermargin(tempinsuranceamount), frequencytypevalue, netperiodicrate),2); //* (1.0 + frequencytypepenalty));
            else
            {
                return Math.Round(calculatePMT(Netpremiumaftermargin(tempinsuranceamount), frequencytypevalue, netperiodicrate), 2); //* (1.0 + frequencytypepenalty));
