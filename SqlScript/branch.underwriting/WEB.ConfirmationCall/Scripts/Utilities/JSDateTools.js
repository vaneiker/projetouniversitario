...
function DateDiff(pFirstDate, pSecondDate, isString, pFormat, pDatePart) {
    ...
    if (typeof pSecondDate === "undefined" || pSecondDate == '')
        pSecondDate = new Date().getMonth() + '/' + new Date().getDay() + '/' + new Date().getFullYear();
    ...
    var result = 0;
    var Days = (pSecondDate - pFirstDate) / (1000 * 60 * 60 * 24) | 0;

    // Years
    if (pDatePart.toLowerCase().indexOf('y') > -1)
        result = Math.floor(Days / 365);
        // Months
    else if (pDatePart.toLowerCase().indexOf('m') > -1)
        result = Math.floor(Days / 31);
        // Days
    else if (pDatePart.toLowerCase().indexOf('d') > -1)
        result = Math.floor(Days);
        // Wrong Datepart
    else
        return false;
