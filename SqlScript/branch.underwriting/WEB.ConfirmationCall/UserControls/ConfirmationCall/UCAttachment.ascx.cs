...
namespace WEB.ConfirmationCall.UserControls.ConfirmationCall
{
    public partial class UCAttachment : UC
    {
        ...
        void Translate()
        {
            ...
            //
            gvAttachedCalls.SettingsText.EmptyDataRow = RESOURCE.UnderWriting.ConfirmationCall.Resources.RowEmpty;
            gvDocuments.SettingsText.EmptyDataRow = RESOURCE.UnderWriting.ConfirmationCall.Resources.RowEmpty;
        }
        ...
        public void FillAttchedDocuments()
        {
            ...
            catch (Exception ex)
            {
                ...
                ConfirmationCallLog.Log("FillAttchedDocuments", ex.Message, parameter, UserDataProvider.UserId.ToInt(), Request.ServerVariables["SERVER_NAME"].ToString());
        ...
        public void FillAttachedCalls()
        {
            ...
            catch (Exception ex)
            {
                ...
                string parameter = "CorpId:" + Step.CorpId + ",RegionId:" + Step.RegionId + ",CountryId:" + Step.CountryId + "DomesticregId:" + Step.DomesticregId + ",StateProvId:"
                    + Step.StateProvId + ",CityId:" + Step.CityId + ",OfficeId:" + Step.OfficeId + ",CaseSeqNo:" + Step.CaseSeqNo + ",Case_Seq_No:" + _services.Case_Seq_No
                    + ",HistSeqNo:" + Step.HistSeqNo + ",StepCaseNo:" + Step.StepCaseNo + ",StepId:" + Step.StepId + "";

                ConfirmationCallLog.Log("FillAttachedCalls", ex.Message, parameter, UserDataProvider.UserId.ToInt(), Request.ServerVariables["SERVER_NAME"].ToString());
        ...
        {
            if (gvAttachedCalls.VisibleRowCount > 0)
            {
        ...
        protected void gvAttachedCalls_RowCommand(object sender, DevExpress.Web.ASPxGridViewRowCommandEventArgs e)
        {
            try
            {
                var KeyValues = e.KeyValue.ToString().Split('|');
                //Session["directoryId"] = e.KeyValue.ToString().Split('|')[1];
                //Session["directoryDetailId"] = e.KeyValue.ToString().Split('|')[2];
                //var IsPrimary = e.KeyValue.ToString().Split('|')[9];
                switch (((Button)e.CommandSource).CommandName)
                {
                    case "View":
                        ...
                        break;
                    case "Delete":
                        diManager.StepManager.DeleteCall(new Policy.Call
                        {
                            ...
                            CommunicationTypeId = KeyValues[4].ToInt(),
                            CallId = KeyValues[3].ToInt(),
