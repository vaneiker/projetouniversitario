//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace DATA.UnderWriting.Data
{
    using System;
    
    public partial class TOBEDROP_SP_GET_GET_UW_REINSURANCE_Result
    {
        public int Corp_Id { get; set; }
        public int Region_Id { get; set; }
        public int Country_Id { get; set; }
        public int Domesticreg_Id { get; set; }
        public int State_Prov_Id { get; set; }
        public int City_Id { get; set; }
        public int Office_Id { get; set; }
        public int Case_Seq_No { get; set; }
        public int Hist_Seq_No { get; set; }
        public int Policy_Status_Id { get; set; }
        public int Insured_Id { get; set; }
        public Nullable<int> Product_Id { get; set; }
        public Nullable<int> Policy_Serie_Id { get; set; }
        public int Company_Id { get; set; }
        public Nullable<int> Agent_Id { get; set; }
        public Nullable<int> Chain_Det_Id { get; set; }
        public Nullable<int> Rider_id { get; set; }
        public Nullable<int> Rider_Status_Id { get; set; }
        public Nullable<int> Rider_Type_Id { get; set; }
        public Nullable<int> Reinsurance_Id { get; set; }
        public string Policy_No { get; set; }
        public string Policy_Status_Desc { get; set; }
        public string Serie_Code { get; set; }
        public string Serie_Desc { get; set; }
        public string Product_Desc { get; set; }
        public string First_Name { get; set; }
        public string Middle_Name { get; set; }
        public string Lastname { get; set; }
        public Nullable<decimal> Benefit_Amount { get; set; }
        public string Global_Country_Desc { get; set; }
        public string Office_Desc { get; set; }
        public string Company_Desc { get; set; }
        public string AssigedTo { get; set; }
        public string Priority { get; set; }
        public Nullable<System.DateTime> Submit_Date { get; set; }
        public Nullable<System.DateTime> Policy_Effective_Date { get; set; }
        public string Agent_First_Name { get; set; }
        public string Agent_Second_Lastname { get; set; }
        public string Agent_Middle_Name { get; set; }
        public string Agent_First_Lastname { get; set; }
        public string Step { get; set; }
        public string Rider_First_Name { get; set; }
        public string Rider_Middle_Name { get; set; }
        public string Rider_Second_Lastname { get; set; }
        public string Rider_Lastname { get; set; }
        public Nullable<System.DateTime> DateSentToReinsurance { get; set; }
        public Nullable<int> TimeinReinsurance { get; set; }
        public string Reinsurer { get; set; }
        public Nullable<decimal> Reinsurance_Amount { get; set; }
        public Nullable<System.DateTime> Reinsurance_Date { get; set; }
    }
}
