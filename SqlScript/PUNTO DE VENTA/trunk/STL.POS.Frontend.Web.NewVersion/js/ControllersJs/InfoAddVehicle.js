InitializeVehicle = function () {
    ...
    $("#noPolicyDecla").focusout(function () {
        var $this = $(this);
        if ($this.val() !== '') {
            getPolicyInfo();
        }
    ...
    $("#noPolicyDecla").trigger("focusout");
    ...
    $("#frmVehicle").validate(
        {
            rules: {
                ...
                ddlPaymentFreq: {
                    required: function () {
                        ...
                        var isVisible = $("#ddlPaymentFreq").is(":visible");

                        if (GlobalAppMode == "LEYMODE" || !isVisible) {
                            return false;
                        }
                        return true;
                    }
            ...
            messages: {
                ...
                noPolicyDecla: {
                    required: 'El Número de Póliza Declarativa es requerida.'
                }
            }
        });
}
...
function getPolicyInfo() {
    ...
    if (qid > 0 && pnumber) {
        ...
        $.ajax({
            ...
            type: "POST",
            data: { policyNo: pnumber, quotationId: qid },
            async: true,
            success: function (data, textStatus, jqXHR) {

                if (data.code === '000') {
                    pass = true;
                    $("#btnSaveVehicleAndNext").removeAttr('disabled');

                    if ($("#btnSendToGlobal").length > 0) {
                        $("#btnSendToGlobal").removeAttr('disabled');
                        $("#btnSendToGlobal").removeClass('ui-state-disabled');
                    }
                    //$("#EndDate").val(data.realEndDate);
                }
                else {
                    showError([data.msg]);

                    $("#btnSaveVehicleAndNext").attr('disabled', 'disabled');

                    if ($("#btnSendToGlobal").length > 0) {
                        $("#btnSendToGlobal").attr('disabled', 'disabled');
                        $("#btnSendToGlobal").addClass('ui-state-disabled');
                    }
                }
            },
            error: function (data, textStatus, jqXHR) {
