...
namespace STL.POS.WsProxy
{
    public interface ICoreProxy
    {
        ...
        PolicyMinimoDeducibleResult GetMinimoDeducible(int? compania, int? ramo, int? subRamo, int? iNTERMEDIARIO, int? year, int? mODELO, int? secuencia, int? IdUso);

        IEnumerable<PolicyReglasValidacion> GetReglasValidacion(Nullable<decimal> cotizacion, string cotizacionPOS);
    }
}
