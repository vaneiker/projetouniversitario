    ...
    public class LocalidadesController : BaseController
    {
        ...
        public ActionResult CreateProvincies(int? ID)
        {
            try
            {
                if (ID != null)
                {
                    ...
                    ViewBag.Zonas = new SelectList(zonas, "Codigo", "Descripcion", ProvSysFlex[0].CodZona);
        ...
        public ActionResult CreateProvincies(Paises_VM paises_VM)
        {
            try
            {
                ...
                if (Session["Nombre_Prov"].ToString() == "Actualizar")
                {
                    ...
                    var prov =DataAccess.Mostrar_Provincias_x_Pais().Where(x=>x.Country_Id==paises_VM.Paises).ToList();
                    ...
                    var ProvSysF = DataAccess.All_Provincias().Where(x => x.CodPais == CodPais).ToList();

                    //Global
                    DataAccess.Modificar_Provincias(Convert.ToInt32(paises_VM.Paises), Convert.ToInt32(prov[0].State_Prov_Id), paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), "", "Global", 0);
                    //POS_SITE
                    DataAccess.Modificar_Provincias(Convert.ToInt32(paises_VM.Paises), Convert.ToInt32(prov[0].State_Prov_Id), paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), "", "POS_SITE", 0);
                    //POS_SITE_ATL
                    DataAccess.Modificar_Provincias(Convert.ToInt32(paises_VM.Paises), Convert.ToInt32(prov[0].State_Prov_Id), paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), "", "POS_SITE_ATL", 0);
                    //SysFlexSeguros
                    DataAccess.Modificar_Provincias(CodPais, ProvSysF[0].Codigo, paises_VM.Nombre_Prov, 0, Session["UserName"].ToString(), "SysFlexSeguros", Convert.ToInt32(CodZona));
                ...
                else
                {
                    ...
                    var Nombrepais = DataAccess.Mostrar_Pais().Where(x => x.Global_Country_Id == paises_VM.Paises);
                    //Saco codigo del pais de Sysflexs
                    var datos = DataAccess.All_PaisesSysFlex().Where(x => x.Descripcion == paises_VM.Nombre_Pais.ToUpper());
                    var CodPais = datos.ToList()[0].Codigo;
                    var CodZona = Request["Zonas"];
                    ...
                    //Provincia Global
                    DataAccess.Registrar_Provincias_All("Global", paises_VM.Paises, paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), 0, 0, "");
                    //Provincia POS_SITE
                    DataAccess.Registrar_Provincias_All("POS_SITE", paises_VM.Paises, paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), 0, 0, "");
                    //Provincia POS_SITE_ATL
                    DataAccess.Registrar_Provincias_All("POS_SITE_ATL", paises_VM.Paises, paises_VM.Nombre_Prov, Convert.ToInt32(Session["Usuario"].ToString()), 0, 0, "");
                    //Provincia SysFlexSeguros
                    DataAccess.Registrar_Provincias_All("SysFlexSeguros", paises_VM.Paises, paises_VM.Nombre_Prov, 0, CodPais, int.Parse(CodZona), Session["UserName"].ToString());

                    Success(string.Format("Ha sido creada  la Provincia <b>{0}</b> ", paises_VM.Nombre_Prov), true);
                    return RedirectToAction("Provincies");
                }
            }
            catch (Exception)
        ...
        public ActionResult Municipios()
        {
            try
            {
                ViewBag.CurrentPage = "Localidades";
                dynamic model = new ExpandoObject();
                model.Municipios = DataAccess.All_Municipios();
                return View(model);
            }
            catch (Exception)
            {
        ...
        [HttpGet]
        public ActionResult CreateMunicipios(int? ID)
        {
            try
            {
                if (ID != null)
                {
                    var datos = DataAccess.All_Municipios().Where(x => x.Codigo == ID).ToList();
                    var idPais = datos[0].Country_Id;
                    var idProv = datos[0].State_Prov_Id;
                    var CodZona = datos[0].CodZona;
                    ...
                    Paises_VM paises_VM = new Paises_VM();
                    paises_VM.Nombre_Municipio = datos[0].Descripcion;
                    ...
                    var Zona = DataAccess.All_Zonas().Where(x => x.Codigo == CodZona).ToList();
                    ViewBag.Zonas = new SelectList(Zona, "Codigo", "Descripcion", CodZona);
                ...
                else
                {
                    ...
                    Llenar_Paises();
                    Llenar_Zonas();
                    //var provincias = DataAccess.Mostrar_Provincias_x_Pais().ToList();
                    //ViewBag.Provincias = new SelectList(provincias, "State_Prov_Id", "State_Prov_Desc", null);
                    //ViewBag.Provincias = new SelectList(listbrd, "brd_cod", "brd_mei", null);
                    var Datos = DataAccess.Mostrar_Provincias_x_Pais().ToList();
                    ...
                    ViewBag.Provincias = new SelectList(Datos.Where(c => c.State_Prov_Id == 0), "State_Prov_Id", "State_Prov_Desc");
        ...
        [HttpPost]
        public ActionResult CreateMunicipios(Paises_VM paises_VM)
        {
            ...
            try
            {
                ...
                //Busco codigo de Pais de SysFlexs
                var Nombrepais = DataAccess.All_Provincias().Where(x => x.Descripcion == paises_VM.Nombre_Prov.ToUpper()).ToList();
                var CodPais = Nombrepais[0].CodPais;
                var CodProv = Nombrepais[0].Codigo;
                var CodZona = Request["Zonas"];

                //Municipio en Global
                DataAccess.Registrar_Municipios_All("Global", paises_VM.Paises, paises_VM.Provincias, paises_VM.Nombre_Municipio, Convert.ToInt32(Session["Usuario"]), 0, 0, 0, "");

                //Municipio SysFlexs
                DataAccess.Registrar_Municipios_All("SysFlexSeguros", 0, 0, paises_VM.Nombre_Municipio, 0, CodPais, Convert.ToInt32(CodZona), CodProv, Session["Usuario"].ToString());
                

                Success(string.Format("Ha sido creado el Municipio <b>{0}</b>  en la Provincia <b>{1}</b>", paises_VM.Nombre_Municipio, paises_VM.Nombre_Prov), true);
                return RedirectToAction("Zonas", "Municipios");
            }
            catch (Exception)
            {
