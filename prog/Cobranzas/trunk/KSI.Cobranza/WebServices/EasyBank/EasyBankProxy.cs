using KSI.Infrastructure.EasyBank;
using System;
...
namespace WebServices.EasyBank
{
    ...
    {
        private KSI.Infrastructure.EasyBank.WsKCOClient EasybankServiceClient
        {
            get { return new Lazy<KSI.Infrastructure.EasyBank.WsKCOClient>().Value; }
        }
        public EasyBankProxy() { }

        public DetalleRecibo ApplyPayToEasyBank(string Account, decimal MontoAplicadoEasybank, int Canal)
        {
            var result = EasybankServiceClient.RecibirPago(Account, MontoAplicadoEasybank, Canal);
            return
                result;
        }     
