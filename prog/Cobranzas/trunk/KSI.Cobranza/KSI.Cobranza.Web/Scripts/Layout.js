...
function SelectActivePage(Grid) {
    //var $Td = $("#" + Grid + " tfoot td:first");
    ////Obtengo el html de los a
    //var $todosLosA = $Td.find("a");
    ////Remover todos los a
    //$Td.find("a").remove();
    ////Obtengo el texto que quedo
    //var PageNumber = parseInt($Td.text());
    //var PageSelectedHtml = "<span>" + PageNumber + "</span>";

    //$todosLosA.splice((PageNumber - 1), 0, PageSelectedHtml);

    //for (var i = 0; i < $todosLosA.length; i++) {
    //    $Td.append($todosLosA[i]);
    //}
}
