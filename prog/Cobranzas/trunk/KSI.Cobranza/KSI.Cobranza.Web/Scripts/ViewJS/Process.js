...
function GetViewNotes(accountId, relatedContactId, CustomerName, LoanNumber) {
    GetPartialView("/Process/_Notes", JSON.stringify({ relatedContactId: relatedContactId, accountId: accountId, CustomerName: CustomerName, LoanNumber: LoanNumber }), function (data) {
        ...
        $Container.html(data);
        ConfigGridCases(accountId);
    }, true);
}

function ConfigGridCases(accountId) {
    var $gvCases = $("#gvCases");
...
function SetGridCalls() {
    var CallRexDate = $("#txtCallDate").val();
     $("#gvPhoneCalls tfoot a").each(function () {
         var $this = $(this);
         var url = "/Process/_GridCallRexCall?page=" + $this.text() + "&CallRexDate=" + CallRexDate;
         $this.attr("href", url);
     });
}
...
function RefreshGridCases(accountId, relatedContactId, CustomerName, LoanNumber) {
    ...
    GetPartialView("/Process/_GridCases", JSON.stringify({ relatedContactId: relatedContactId, accountId: accountId, CustomerName: CustomerName, LoanNumber: LoanNumber, CaseNumber: CaseNumber }), function (data) {
...
function ShowPopMakeNote(CustomerId, CustomerName, LoanNumber, AccountId) {
    ...
    if ($inputChk.length > 0) {
        ...
        IsClosed = $inputChk.attr('status') == "Cerrado";
        if (IsClosed)
        {
            ShowMessageBS("Este caso esta cerrado", "Advertencia", null, null);
            return false;
        }
    ...
    GetPartialView("/Process/_MakeNote", JSON.stringify({ CustomerId: CustomerId, CustomerName: CustomerName, LoanNumber: LoanNumber, accountId: AccountId, IsNewcase: IsNewcase }), function (data) {
        ...
        $("#DepartmentId").change(function () {
            var $this = $(this);
            var $QueueTypeId = $("#QueueTypeId");
            if ($this.val() != "") {
                CallAjax("/Process/GetQueueType", JSON.stringify({ DepartmentId: parseInt($this.val()) }), "json", function (data) {
                    var options = "";
                    $QueueTypeId.find("option").remove();
                    var FirstOption = "<option value=''>Seleccionar</option>";
                    $QueueTypeId.append(FirstOption);
                    $.each(data, function (i, v) {
                        var option = "<option value='" + v.Key + "'>" + v.Value + "</option>";
                        options += option;
                    });
                    $QueueTypeId.append(options);
                    $QueueTypeId.removeAttr("disabled");
                }, "POST", false);
            } else {
                $QueueTypeId.find("option").remove();
                $QueueTypeId.attr("disabled", "disabled");
            }
        });
...
function SearchCallRexCall() {

    var CallRexDate = $("#txtCallDate").val();
    GetPartialView("/Process/_GridCallRexCall", JSON.stringify({ CallRexDate: CallRexDate }), function (data) {
        var $Container = $("#dvCallRexCall");
        $Container.html(data);
        SetGridCalls();
