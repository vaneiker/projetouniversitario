...
namespace KSI.Cobranza.Web.Controllers
{
    public class ProcessController : BaseController
    {
        ...
        public PartialViewResult _GridCases(Nullable<long> relatedContactId, Nullable<long> accountId, string CustomerName, string LoanNumber,long? CaseNumber = null)
        {
            var model = _oProcessModel.GetCasesViewModel(relatedContactId, accountId, CaseNumber, null, CustomerName, LoanNumber).Cases;
            return
                PartialView(model);
        }
        ...
        public PartialViewResult SetMeegintNote(MeetingNoteViewModels model)
        {
            if (ModelState.IsValid)
            {
                _oProcessModel.SetMeeting(model, oTemDataNoteAttachedFiles);
            ...
            var modelresult = _oProcessModel.GetMeetingModel(model.relatedContactId, model.CustomerName, model.LoanNumber, model.accountId,true);
            return
        ...
        public void PrintListCases(string ExportType, Nullable<long> relatedContactId, Nullable<long> accountId, string CustomerName, string LoanNumber)
        {
            var ExpType = (Enums.ExportType)Enum.Parse(typeof(Enums.ExportType), ExportType);
            var model = _oProcessModel.GetCasesViewModel(relatedContactId, accountId, null, null, CustomerName, LoanNumber);
            var grid = new WebGrid(source: model.Cases, canPage: false, canSort: false);
            WebGridColumn[] Columns = {
                                           grid.Column("CaseNumberF", "Numero de Caso"),
                                           grid.Column("CaseDate", "Fecha"),
                                           grid.Column("OpeningCase", "Apertura Caso"),
                                           grid.Column("ClosingCase", "Cierre Caso"),
                                           grid.Column("CaseType", "Servicio"),
                                           grid.Column("CaseSubType", "Sub Servicio"),
                                           grid.Column("CategoryCase", "Categoria"),
                                           grid.Column("AboutCase", "Respecto A"),
                                           grid.Column("User", "Usuario"),
                                           grid.Column("Recording", "Grabación"),
                                           grid.Column("NoteNumber", "# Nota"),
                                           grid.Column("Priority", "Prioridad")
                                      };

            base.PrintExcel(grid, Columns, ExpType, "Casos");
        }

        public PartialViewResult _MakeNote(long? CustomerId, string CustomerName, string LoanNumber, long? accountId, bool IsNewcase)
        {
            var model = _oProcessModel.GetMeetingModel(CustomerId, CustomerName, LoanNumber, accountId,IsNewcase);
            return
                PartialView(model);
        }
        ...
        public ActionResult GetCallRecording(string fileName)
        {
            var FullPath = Path.Combine(base.CallRexPath, fileName);
            return File(base.isDebug ? @"\Audio\Horse_1.wav" : FullPath, "audio/mp3");
        }

        public PartialViewResult _GridCallRexCall(DateTime? CallRexDate)
        {
            var model = new List<PhoneCallViewModels>(0);
            if (CallRexDate.HasValue)
                model = _oProcessModel.GetPhoneCall(base.CallRexPath, base.isDebug, base.GetCurrentUserID(), CallRexDate.GetValueOrDefault()).ToList();
        ...
        [HttpPost]
        public JsonResult GetQueueType(int DepartmentId)
        {
            var QueueTypeIds = new string[] { "1", "2" };
            var data = _oProcessModel.GetDropDown(Enums.DropDownType.QueueType, DepartmentId).Where(x => QueueTypeIds.Contains(x.Key));
            return
                Json(data, JsonRequestBehavior.AllowGet);
        }
