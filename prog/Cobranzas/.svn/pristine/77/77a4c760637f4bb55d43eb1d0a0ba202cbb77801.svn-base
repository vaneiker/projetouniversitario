...
namespace KSI.Cobranza.DataLayer.Repositories
{
    public class LoanRepository : BaseRepository, IBaseRepository<Loan>
    {
        ...
        public Loan.Queue SetQueue(Nullable<long> queueId, Nullable<long> accountId, Nullable<int> queueTypeId, Nullable<int> distributionId, Nullable<int> submittedByDepartmentId, Nullable<int> transferredFromDepartmentId, Nullable<int> queueStatusId, Nullable<long> vendorId, Nullable<long> assingToId, Nullable<long> transactionPaymentPlanId, Nullable<int> queueSourceId, string accountReference, Nullable<int> daysLate, Nullable<System.DateTime> actionDate, Nullable<System.DateTime> endDate, Nullable<bool> isActive, string cOMMENTS, Nullable<int> userId, string userName)
        {
            Loan.Queue result;
            var temp = _dbContext.SP_SET_QUEUE(queueId,
                                               accountId,
                                               queueTypeId,
                                               distributionId,
            ...
            result = temp.Select(c => new Loan.Queue
            {
                Action = c.Action,
                QueueId = c.QueueId
            })
        ...
        public Loan.CallUser GetUsersCall(Nullable<int> securityUserId)
        {
            Loan.CallUser result;
            IEnumerable<SP_GET_USER_CALL_Recording_ID_Result> temp = _dbContext.SP_GET_USER_CALL_Recording_ID(securityUserId);
            result = temp.Select(c => new Loan.CallUser
            {
                SecurityUserId = c.SecurityUserId,
                UserIdForCallsRecording = c.UserIdForCallsRecording,
                UserName = c.UserName,
                CreateDate = c.CreateDate,
                CreateUsr = c.CreateUsr,
                ModiDate = c.ModiDate,
                ModiUsr = c.ModiUsr,
                Active = c.Active,
        ...
        /// <param name="accountId"></param>
        /// <returns></returns>
        public IEnumerable<Loan.Document> GetDocuments(long accountId)
        {
            return
                _dbContext.GET_DOCUMENTS(accountId, null)
                .Select(x => new Loan.Document() {
                    accountId = x.accountId,
        ...
        /// <returns></returns>
        public Loan.Document GetDocuments(long accountId, int IdRequirement)
        {
            return
                _dbContext.GET_DOCUMENTS(accountId, IdRequirement)
                .Select(x => new Loan.Document()
                {
                    accountId = x.accountId,
                    IdRequirement = x.IdRequirement,
