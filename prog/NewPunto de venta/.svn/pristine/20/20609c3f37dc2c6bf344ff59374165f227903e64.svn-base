...
$(document).ready(function () {
    ...
    $("#spVersionApp").html("(App Ver. 22.5.0)");
    //
    ...
    $("#frmVehicleInfoBasic").validate(
        {
            ...
            messages: {
                ...
                fuelType: {
                 ...
                 contactForm: {
                     required: 'El campo Vía de Contacto es requerido.'
                }
            }
        });
}
            ...
            $.each(quotData._vehicles, function (idx, obj) {
                ...
                if (vehicle != undefined) {
                    ...
                    if (GlobalAppMode != "LEYMODE") {
                        ...
                        //GetFuelType(null, vehicle.VehicleModel_Make_Id, vehicle.VehicleModel_Model_Id);
                    ...
                    if (vehicle.SurChargePercentage > 0) {
                        getOptionsByValueOfDropWithClassAndSelect('.SurchargePercent', randomID, vehicle.SurChargePercentage);
                    }
...
function GetURLParameter() {
    ...
    //if (sPageURL.toLowerCase().indexOf('ley')!==-1) {
    //    SetLeyMode("N");
    //}
...
function validateGetRates(randomID) {
    ...
    if (driver == 0 && GlobalAppMode != "LEYMODE")
...
function setVehicle(obj, randomID) {
    ...
    if (GlobalAppMode == "LEYMODE") {
        ...
        newVehicle.SelectedVehicleFuelTypeId = 0;
        newVehicle.SelectedVehicleFuelTypeDesc = "";
        ...
        $.each(vehicles, function (idx, v) {
            ...
            if (vehicleRandomID == v.randomId) {
                ...
                if ($("#SurchargePercent").length > 0) {
                    ...
                    surchargeDrop = '<li class="mdl-list__item mdl-card__actions mdl-card--border HideOnLeyMode">' +
                ...
                var detail = '<div class="mdl-card__supporting-text w-100">' +
                    ...
                    '<strong class="mdl-list__item-primary-content">Descripcion Cobertura:</strong>' +
                ...
                detail = detail.replace("{1}", vehicledesc).replace("{2}", vRandomID)
                    ...
                    .replace("{15}", vRandomID).replace("{15}", vRandomID).replace("{15}", vRandomID).replace("{15}", vRandomID).replace("{15}", vRandomID)
                    ;
...
function getOptionsByValueOfDropWithClassAndSelect(dropClass, randomID, value) {
...
function SaveBasicClientData() {
    ...
    if ($("#frmClientInfoBasic").valid() == false && GlobalAppMode != "LEYMODE") {
        return false;
    }
    ...
    if (GlobalAppMode == "LEYMODE") {
        ...
        //Cliente por default
        newJson.FirstName = DefaultClient.Name; //$("#FirstName").val();
        newJson.FirstSurname = "";
        newJson.DateOfBirth = DefaultClient.DateOfBirth; //$("#DateOfBirth").val();
        newJson.Sex = DefaultClient.Sex;//$("#Sex").val();
        newJson.ForeignLicense = "NO"; //$("#ForeignLicense").val();
        ...
        success: function (result) {
            ...
            switch (result.optionSelected) {
                ...
                default:
                    ...
                    $("#hdnDefaultCupon").val(result.couponDefault);
...
function GetFuelType(fueltypeid, makeid, modelid) {
...
function configAppMode() {
    ...
    if (GlobalAppMode == "LEYMODE") {
        ...
        $("#spanNumberStep2").text("1");
        $("#spanNumberStep3").text("2");

    } else {
        $(".HideOnLeyMode").show();
        $(".HideOnNotLeyMode").hide();
