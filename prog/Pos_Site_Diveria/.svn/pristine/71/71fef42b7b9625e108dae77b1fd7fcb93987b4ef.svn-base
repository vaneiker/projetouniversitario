...
namespace STL.POS.Data.NewVersion.Repository
{
    public class UserRepository : BaseRepository
    {
        ...
        public AgentByUser GetAgentByUser(int? UserId)
        {
            AgentByUser result;
            IEnumerable<SP_GET_AGENT_BY_USER_Result> temp;
            temp = PosContex.SP_GET_AGENT_BY_USER(UserId);

            result = temp.Select(q => new AgentByUser()
            {
                Agent_Code = q.Agent_Code,
                User_Id = q.User_Id
            }).FirstOrDefault();
