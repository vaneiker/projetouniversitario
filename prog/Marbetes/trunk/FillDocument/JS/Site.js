...
/// <reference path="../MarbeteWeb.aspx" />

function pageLoad() {
    $("div.preloader").show();
    var btn = document.getElementById('btnFillPDF');
    btn.value = "Generar";
    var policies = document.getElementById("ddlPolicies");
    SetCombine(policies);
         
    //    var objeto = document.getElementById("btnReset");
...
function ValidatePolicies() {
    pageLoad();
    var btn = document.getElementById('btnFillPDF');
    var dropdpwn = document.getElementById("ddlPolicies");
    var policies = document.getElementById("txtPolicies").value;
    if (policies == '') {
        alert("Debe agregar al menos una poliza!!");
        $("div.preloader").hide();
        return false;
    }
    if (dropdpwn.value == "7") {
        if (ValidateCheckBoxes()) {
            alert("Debe cotejar al menos una sección!!")
            $("div.preloader").hide();
            return false;
        }
    }
...
function SetFull() {
    var checkBox = document.getElementById("cbFull");
    ddlPolicies = document.getElementById("ddlPolicies");
    document.getElementById("ddlPolicies").setAttribute('disabled', false);
    if (checkBox.checked) {
        document.getElementById("hdnIsFull").value = "true";
        ddlPolicies.value = "0";
        ddlPolicies.setAttribute('disabled', false);
        alert("ADVERTENCIA: RECUERDE SELECCIONAR LA IMPRESORA DE MARCA ZEBRA \n PARA LA IMPRESION DE MARBETES DE SEGUROS FULL.\\n\\nDE NO HACERLO PODRIA INCURRIR EN DESPERDICIO DE MATERIAL.");
    }
    else {

        document.getElementById("hdnIsFull").value = "false";
        ddlPolicies.removeAttribute('disabled');
    }
}
...
//    var checkBox = document.getElementById("cbFull");
//    if (checkBox.checked) {
//        alert("ADVERTENCIA: RECUERDE SELECCIONAR LA IMPRESORA MARCA ZEBRA \n PARA LA IMPRESION DE MARBETES DE SEGUROS FULL. \n DE NO HACERLO PODRIA INCURRIR EN DESPERDICIO DE MATERIAL.");
//    }
//    return true;
//}
function AlertToUser() {
    var checkBox = document.getElementById("cbFull");
    if (checkBox.checked) {
        var result = confirm("ADVERTENCIA: RECUERDE SELECCIONAR LA IMPRESORA MARCA ZEBRA \n PARA LA IMPRESION DE MARBETES DE SEGUROS FULL. \nDE NO HACERLO PODRIA INCURRIR EN DESPERDICIO DE MATERIAL. \n \n¿DESEA CONTINUAR?");
        if (result) {
...
function SetCombine(dropdpwn) {
    var txtPolicies = document.getElementById("txtPolicies");
    txtPolicies.style.width = "100%";
    var liElements = document.querySelectorAll('[data-marbete]');
    for (var i = 0; i < liElements.length; i++) {
        liElements[i].style.display = "None";
    }
    if (dropdpwn.value == "7") {
        boxPolicies.style.marginTop = "-68px";
        boxPolicies.style.width = "57.5%";
        boxPolicies.style.marginLeft = "4px";
        for (var i = 0; i < liElements.length; i++) {
        ...
        boxPolicies.style.marginTop = "";
        boxPolicies.style.width = "100%";
        for (var i = 0; i < liElements.length; i++) {
            liElements[i].style.display = "None";
            var chkelement = document.getElementById("marbete" + (i + 1).toString());
...
getLabelField = function (obj) {
    ...
    if (labelObj == null) {
        ...
        Parents.each(function () {
            ...
            var label = label = $(this).find("label:first");
...
validateForm = function (form) {
    ...
    if ($("#hdnValidate").val() == "true") {
        ...
        $("#" + form).find("[validation='Required']").each(function () {
            ...
            if ($this.is("input:text,input:password")) {
                ...
                if (validateEqualControlId != null && validateEqualControlId != "") {
                    if ($("#" + validateEqualControlId).val() != $this.val()) {
                        ...
                        if (ishighlightedFieldRequired) {
                            $this.css("border", "1px solid  #FF0000");
                            $("#" + validateEqualControlId).css("border", "1px solid  #FF0000");
                            $this.keydown(function () {
                                changeBorderColor(this);
                            });
                        }
            ...
            } //Fin Validaciones de input:Text,input:password
            ...
            if ($this.is("span")) {
                ...
                if ($chk != null) {
                    ...
                    if (!$chk.prop('checked')) {
                        ...
                        var Field = getLabelField($this);
                        if (ishighlightedFieldRequired) {
                            $chk.css("border", "1px solid  #FF0000");
                            $chk.keydown(function () {
                                changeBorderColor(this);
                            });
                        }
                        var changeMessage = $chk.attr("changeMessage") != null && $chk.attr("changeMessage") == "true";
...
CustomDialogMessage = function (msj, title) {
    if (document.getElementById("dvMessage")) return false;
    if (!title)
        title = "Advertencia!";
    var divToCreate = "<div id='dvMessage' title='" + title + "'></div>";
    $("body").append(divToCreate);
    ...
    $(divCreated).dialog({
        autoOpen: false,
        resizable: false,
        position: { my: "top", at: "top", of: $(".ncliente") },
        height: 126,
        width: 500,
        modal: true,
        show: { effect: "fadeIn", duration: 260 },
        ...
        open: function () {
            $(divCreated).html(msj);
            //$(".ui-widget-overlay").css("z-index", "1000000");
            //$(".ui-dialog").css("z-index", "10000000000");
        }
    }).dialog("open");
};
