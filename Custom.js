...
$(document).ready(function () {
    ...
    $("#tblHistoric").on("click", ".btnAnular", function () {
        var $btn = $(this);
        showQuestion('¿Esta seguro que desea anular esta factura?', 'Anular Factura', function () { anularInvoice($btn); return false; });
        return false;
    });

});

function anularInvoice($btn) {

    var _info = $btn.data('factinfo');

    debugger
    $.ajax({
        url: "/Queue/AnularInvoice",
        data: { info: _info },
        dataType: 'json',
        async: true,
        success: function (data) {
            ...
            showSucess(['Factura anulada correctamente'], '', function () {
                location.href = "/Queue/History";
            });
